...
class SupportButton(Button):
    ...
    async def create_ticket(self, interaction: Interaction):
        ...
        if modal:
            ...
            if user.avatar:
                form_embed.set_author(name=title, icon_url='https://i.imgur.com/A9kOqMB.png') #Dylberts changes ticket thumbnails to Sofia instead of user opening ticket
            else:
        ...
        if messages:
            ...
            for index, einfo in enumerate(messages):
                ...
                if index == 0:
                    em.set_thumbnail(url='https://i.imgur.com/A9kOqMB.png') #Dylberts thumbnail change
                if einfo["footer"]:
        ...
        with contextlib.suppress(discord.HTTPException):
            if existing_msg:
                ...
                asyncio.create_task(interaction.followup.send(embed=em, ephemeral=True)) #changed
