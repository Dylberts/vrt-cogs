...
class SupportButton(Button):
    ...
    async def create_ticket(self, interaction: Interaction):
        ...
        if modal:
            ...
            if user.avatar:
                form_embed.set_author(name=title, icon_url='https://imgur.com/A9kOqMB') #Dylberts changes ticket thumbnails to Sofia instead of user opening ticket
            else:
        ...
        if messages:
            ...
            for index, einfo in enumerate(messages):
                ...
                if index == 0:
                    em.set_thumbnail(url='https://imgur.com/A9kOqMB') #Dylberts thumbnail change
                if einfo["footer"]:
